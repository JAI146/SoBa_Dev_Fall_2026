...
type AdminTicketDetailModalProps = {
  ...
  onSubmitReply: () => void;
  onUpdateStatus: (status: SponsorTicketStatusValue) => void;
  onClose: () => void;
};
        ...
        {loading || !detail ? (
          ...
          <>
            <div className={styles["sponsorship-detail-header"]}>
              <div>
                <h2 id="admin-ticket-detail-title" className={styles["modal-title"]}>
                ...
                <p className={styles["sponsorship-detail-subtitle"]}>
                  <span className={styles["ticket-detail-meta"]}>
                    <FormIcon name="user" className={styles["ticket-detail-meta__icon"]} />
                    {detail.donorName}
                  </span>
                  <span className={styles["ticket-detail-meta"]}>
            ...
            <div className={styles["ticket-status-actions"]}>
              <button
                type="button"
                className={styles["btn-secondary-inline"]}
                disabled={submitting || detail.status === "in_progress"}
                onClick={() => onUpdateStatus("in_progress")}
              >
                <ProgressIcon />
                {t("sponsorships.tickets.markInProgress")}
              </button>
              <button
                type="button"
                className={styles["btn-primary-inline"]}
                disabled={submitting || detail.status === "resolved"}
                onClick={() => onUpdateStatus("resolved")}
              >
                <CheckIcon />
                {t("sponsorships.tickets.markResolved")}
              </button>
            </div>
            ...
            {detail.status !== "resolved" ? (
              <div className={styles["ticket-reply-form"]}>
                <label htmlFor="admin-ticket-reply">
                  <FormFieldLabel icon="text">
                    {t("sponsorships.tickets.adminReplyLabel")}
                  </FormFieldLabel>
                </label>
                <textarea
                  ...
                  placeholder={t("sponsorships.tickets.adminReplyPlaceholder")}
                  disabled={submitting}
                />
                <div className={styles["modal-actions"]}>
                  ...
                  >
                    ...
                    {submitting
                      ...
                      : t("sponsorships.tickets.adminReplyCta")}
                  </button>
                </div>
              </div>
            ) : (
