...
export default function DonorChatRoomPage() {
  const { t } = useI18n();
  ...
  const loadRoom = useCallback(async () => {
    ...
    try {
      ...
      const data = await getChatRoom("donor", roomId, token);
      setRoom(data.room);
    } catch (err) {
  ...
  if (error || !room) {
    return (
      <div className={styles.card}>
        ...
        <Link href="/dashboard/visitor/chat" className={styles["back-link"]}>
          {t("chat.backToRooms")}
        </Link>
      </div>
    );
  ...
  return (
    <>
      <div className={styles["page-top"]}>
        <BackLink href="/dashboard/visitor/chat">{t("common.back")}</BackLink>
      </div>
      <ParticipantChatPanel role="donor" room={room} />
    </>
  );
}
