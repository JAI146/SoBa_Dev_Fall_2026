...
export default function FamilyChatPage() {
  ...
  const loadRooms = useCallback(
    async (opts?: { silent?: boolean }) => {
      ...
      try {
        ...
        const data = await listChatRooms("family", token);
        setRooms(data.rooms);
      } catch (err) {
  ...
  const filteredRooms = useMemo(
    () => filterChatRooms(rooms, search, "family"),
    [rooms, search],
  );
    ...
    <>
      <div className={styles["page-actions"]}>
        <div className={styles["page-header"]} style={{ marginBottom: 0 }}>
          <h1>{t("chat.familyTitle")}</h1>
          <p className={styles["page-description"]}>{t("chat.familyDescription")}</p>
        </div>
        <RefreshButton
      ...
      <ChatInboxPageShell>
        {!loading && rooms.length > 0 && (
          <ChatInboxSearch
            value={search}
            onChange={setSearch}
            placeholder={t("chat.searchDonorPlaceholder")}
          />
        )}
        ...
        <div className={`${styles.card} ${styles["card-wide"]} ${styles["chat-inbox-card"]}`}>
          ...
          {!loading && rooms.length === 0 && (
            <p className={styles["chat-inbox-empty"]}>{t("chat.noFamilyRooms")}</p>
          )}
          ...
          {!loading && filteredRooms.length > 0 && (
            <ChatRoomInboxList
              ...
              viewerRole="family"
              getRoomHref={(room) =>
                `/dashboard/family/chat/${encodeURIComponent(room.id)}`
              }
            />
          )}
        </div>
